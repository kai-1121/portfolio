...
import pandas as pd
file_path = "/home/user/Users/kai/data/pose_results/a_school/sub_tn/0947-0957/cleaned/"
...
def rmse(data):
    rmse = np.sqrt(np.mean((data - np.mean(data)) ** 2))
    return rmse
...
def Unified_average_speed(average_speed,fps):
    unified_average_speed=np.mean(average_speed)*fps
    return unified_average_speed
     
def short_form_area(data_x,data_y):
    maximum_width_x=np.max(data_x)-np.min(data_x)
    maximum_width_y=np.max(data_y)-np.min(data_y)
    short_form_area=maximum_width_x*maximum_width_y
    return maximum_width_x,maximum_width_y,short_form_area
...
lll=[]
kkk=[]
lll.append(np.mean(data_0[:,3:4,0:1]))
lll.append(np.mean(data_0[:,4:5,0:1]))
kkk.append(np.mean(data_0[:,3:4,1:2]))
kkk.append(np.mean(data_0[:,4:5,1:2]))

x_sholder=list(map(float, lll))
y_sholder=list(map(float, kkk))
# x_sholder=list(map(float, data_0[10,3:5,0:1]))
# y_sholder=list(map(float, data_0[10,3:5,1:2]))
...
after_mov_avg_data_0_x=normalization(after_mov_avg_data_0_x,shoulder_width)
after_mov_avg_data_0_y=normalization(after_mov_avg_data_0_y,shoulder_width)
...
data_0_unified_average_speed=Unified_average_speed(data_0_average_speed,fps=30)
print(f"\ndata_0_unified_average_speed:{data_0_unified_average_speed}")
...
data_0_rmse=rmse(data_0_average_speed)
print(f"\ndata_0_rms:{data_0_rms}\ndata_0_rmse:{data_0_rmse}")

data_0_maximum_width_x,data_0_maximum_width_y,data_0_short_form_area=short_form_area(after_mov_avg_data_0_x,after_mov_avg_data_0_y)
print(f"data_0_maximum_width_x:{data_0_maximum_width_x},data_0_maximum_width_y:{data_0_maximum_width_y},data_0_short_form_area:{data_0_short_form_area}")
